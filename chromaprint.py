...
ALGORITHM_DEFAULT = Algorithms.ALGORITHM_TEST2
...
class Fingerprinter:
    ...
    def start(self, sample_rate: int, num_channels: int):
    ...
    def feed(self, data: memoryview | bytearray | bytes):
...
def decode_fingerprint(data: bytes, base64: bool = True):
...
def encode_fingerprint(fingerprint: bytes, algorithm: Algorithms, base64: bool = True):
...
def hash_fingerprint(fingerprint: Sequence[int]):
